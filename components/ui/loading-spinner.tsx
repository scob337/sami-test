...
export function LoadingSpinner({ size = "md", className }: LoadingSpinnerProps) {
  const sizeClasses = {
    ...
    lg: "w-40 h-40 gap-8",
    xl: "w-60 h-60 gap-10",
    "2xl": "w-80 h-80 gap-12"
  }

  return (
    <div className={cn("flex flex-col items-center justify-center", sizeClasses[size].split(" ")[2] || "gap-10", className)}>
      <motion.div
        animate={{
          scale: [1, 1.02, 1],
        }}
      ...
      {/* Loading Text - Reduced for smaller sizes */}
      {size !== "sm" && (
        <div className="flex flex-col items-center gap-4">
          ...
          >
            <motion.h2
              ...
              className={cn(
                "font-black text-transparent bg-clip-text bg-gradient-to-r from-primary via-blue-400 to-accent tracking-[0.1em] px-4",
                size === "md" ? "text-xl md:text-2xl" : "text-3xl md:text-4xl"
              )}
            >
              جاري التحميل
            </motion.h2>
            
            {/* Decorative dots fade out on small */}
